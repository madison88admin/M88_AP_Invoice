...
async function extractInvoiceFieldsFromText(text: string, fileBuffer?: Buffer) {
  ...
  // Proforma-invoice layouts detach the number from its "P/I NO." /
  // "ORDER NO." labels — pdf2json flattening interleaves column text
  // ("P/I NO. UNIT PRICE DENVER CO, 80205 ... A40319 DATE") and OCR engines
  // emit one token per line ("ORDER NO.\nPAYMENT TERMS\nMARMOT\nA40319").
  // No label-adjacent pattern can bridge that, so when the document is a
  // proforma invoice scan the whole text for a letter-leading number token,
  // excluding every other numbered namespace (PO/MPO/SO/…).
  if (!invoice_number && /PROFORMA\s*INVOICE/i.test(text.replace(/\s+/g, ''))) {
    const tokenRe = /\b([A-Z]{1,4}\d[A-Z0-9\-/]{2,})\b/g;
    // Values belonging to other numbered namespaces (also matched when OCR
    // glues the namespace to the number, e.g. "MARMOTPO03036MPO015937MLINDO").
    const notInvoice = /(?:MPO|PO|SO|S\/O|BOM|CAT|PF|OCT|VNS|HS|CL|PL|SI)\s*[:#-]?\s*[A-Z]*\d/i;
    let tm: RegExpExecArray | null;
    while ((tm = tokenRe.exec(text)) !== null) {
      const token = tm[1];
      // Context guard: skip tokens belonging to other namespaces.
      const before = text.substring(Math.max(0, tm.index - 16), tm.index + 4);
      if (notInvoice.test(before)) continue;
      // Skip bare numeric/dated tokens (letter-leading is already required).
      if (/^\d+([.,]\d+)?$/.test(token)) continue;
      if (/^[A-Z]{2}(19|20)\d{2}$/.test(token)) continue;
      invoice_number = token;
      logger.info(`[OCR] Invoice number from proforma layout: ${invoice_number}`);
      break;
    }
  }
