...
function makePayment(overrides: Record<string, any> = {}) {
  ...
  return {
    ...
    invoice: {
      ...
      invoice_date: invoice.invoice_date ?? new Date('2026-08-01'),
      due_date: invoice.due_date ?? null,
      currency: invoice.currency ?? 'USD',
      total_amount: invoice.total_amount ?? overrides.amount ?? 100,
      brand: invoice.brand ?? 'TEST_BRAND',
      season: invoice.season ?? 'SS27',
      payment_terms: invoice.payment_terms ?? '30 DAYS',
      raw_file_url: invoice.raw_file_url ?? 'https://example.test/invoice.pdf',
