...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // have recovered, or the first run stored a snapshot and needs a real comparison.
    ...
    const validate = async () => {
      ...
      try {
        ...
        if (data?.nextGenUnavailable) {
          ...
          // mismatches still surface in red.
          if (data.hasCriticalChanges) {
            ...
            setNextgenResults(prev => ({ ...prev, [selectedInvoice.id]: { status: 'baseline', changes: data.changes, data } }));
          }
                  ...
                  <div className="p-4 rounded-xl" style={{ background: 'color-mix(in srgb, var(--accent-blue) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--accent-blue) 20%, transparent)' }}>
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium" style={{ color: 'var(--accent-blue)' }}>NextGen baseline stored — re-check to verify</span>
                      </div>
                      <button
                        onClick={retryNextGenCheck}
                        className="text-xs font-medium whitespace-nowrap"
                        style={{ background: 'var(--accent-blue)', color: 'var(--text-inverse)', padding: '6px 12px', borderRadius: '8px', border: 'none', cursor: 'pointer' }}
                      >
                        Re-check now
                      </button>
                    </div>
