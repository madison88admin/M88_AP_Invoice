...
const apiVendorToMock = (vendor: any): MockVendor => ({
  ...
  bank_name_alt: vendor.bank_name_alt || [],
  bank_address: vendor.bank_address || undefined,
  account_number: vendor.account_number || undefined,
  account_number_alt: vendor.account_number_alt || [],
  swift_code: vendor.swift_code || undefined,
  swift_code_alt: vendor.swift_code_alt || [],
  iban: vendor.iban || undefined,
  sort_code: vendor.sort_code || undefined,
  aba_routing_number: vendor.aba_routing_number || undefined,
  intermediary_bank_name: vendor.intermediary_bank_name || undefined,
  intermediary_bank_swift: vendor.intermediary_bank_swift || undefined,
  has_multiple_accounts: vendor.has_multiple_accounts || false,
  ...
  bir_tin: vendor.bir_tin || undefined,
  eori_number: vendor.eori_number || undefined,
  gstin_number: vendor.gstin_number || undefined,
...
const apiInvoiceToMock = (invoice: any): MockInvoice => {
  ...
  return {
    ...
    mpo_base_number: invoice.mpo_base_number || undefined,
    mpo_order_sequence: invoice.mpo_order_sequence || undefined,
    material_code: invoice.material_code || undefined,
    material_name: invoice.material_name || undefined,
    ...
    confirmation_sent_at: invoice.confirmation_sent_at ? dateToString(invoice.confirmation_sent_at) : undefined,
    ...
    follow_up_tasks: (invoice.follow_up_tasks || []).map((task: any) => ({
      id: task.id || '',
      invoice_id: task.invoice_id || invoice.id || '',
      task_type: task.task_type || '',
      assigned_to: task.assigned_to || '',
      due_date: dateToString(task.due_date),
      status: task.status === 'COMPLETED' ? 'COMPLETED' : 'PENDING',
      reminder_count: Number(task.reminder_count || 0),
      last_reminded_at: task.last_reminded_at ? dateToString(task.last_reminded_at) : undefined,
      completed_at: task.completed_at ? dateToString(task.completed_at) : undefined,
      notes: task.notes || undefined,
      created_at: dateToString(task.created_at),
    })),
    payment_confirmations: (invoice.payment_confirmations || []).map((confirmation: any) => ({
      id: confirmation.id || '',
      invoice_id: confirmation.invoice_id || invoice.id || '',
      email_sent: Boolean(confirmation.email_sent),
      sent_at: confirmation.sent_at ? dateToString(confirmation.sent_at) : undefined,
      created_at: dateToString(confirmation.created_at),
    })),
  };
};
