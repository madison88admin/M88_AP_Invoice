...
export const ROLE_STAGE_ACCESS: Record<string, string[]> = {
  ...
  ACCOUNTING_ASSOCIATE: ['VALIDATION_PENDING', 'APPROVED', 'POSTED_TO_QB', 'PENDING_ACCOUNTING', 'PAYMENT_SCHEDULED', 'ON_HOLD', 'PAID', 'PAYMENT_CONFIRMATION_SENT', 'CANCELLED'],
  ACCOUNTING_SUPERVISOR: ['VALIDATION_PENDING', 'PENDING_ACCOUNTING', 'APPROVED', 'POSTED_TO_QB', 'PAYMENT_SCHEDULED', 'ON_HOLD', 'PAID', 'PAYMENT_CONFIRMATION_SENT', 'CANCELLED'],
...
export function isInvoiceActionableForRole(invoice: any, role: string): boolean {
  ...
  if (['POSTED_TO_QB', 'PAYMENT_SCHEDULED', 'PAID', 'PAYMENT_CONFIRMATION_SENT', 'CANCELLED'].includes(status)) {
    return false;
  }
