...
const localFile = path.join(__dirname, 'tmp-check-detail2.js');
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    const ws = sftp.createWriteStream('/opt/ap-invoice/apps/api/tmp-check-detail2.js');
    ws.on('close', () => {
      conn.exec('cd /opt/ap-invoice/apps/api && node tmp-check-detail2.js 2>&1', (err, stream) => {
