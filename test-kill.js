...
(async () => {
  try {
    // Kill the hanging python process
    console.log('1. Kill hanging process...');
    console.log(await runCmd('kill -9 505376 2>/dev/null || echo "already dead"'));
    
    // Check result
    console.log('\n2. Check result...');
    console.log(await runCmd('cat /tmp/ssh_result.txt 2>/dev/null; echo "---"; ls -la /tmp/ssh_result.txt'));
